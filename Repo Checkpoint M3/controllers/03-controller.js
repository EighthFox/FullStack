...
const updateCar = (car) => {
  ...
  for (let key in car) {
     if (car[key] === undefined) throw Error("Faltan datos a completar");
    }
     if (!utils.cars.find((c) => c.id === car.id))
      throw Error("No se encontro el coche solicitado");
    
    let index = utils.cars.indexOf(utils.cars.find((c) => c.id === car.id));
    utils.cars.splice(index, 1, car);
    return car.id;
};
